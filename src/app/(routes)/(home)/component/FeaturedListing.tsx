...
const Star = ({ filled, onClick }: { filled: boolean; onClick: () => void }) => (
    <svg
        className={`w-4 h-4 cursor-pointer ${filled ? 'text-orange-600 fill-current' : 'text-gray-300'}`}
        onClick={onClick}
    ...
    return (
        <div className="py-10 max-w-screen-xl mx-auto">
            <div className="flex flex-col justify-center items-center text-center mb-10">
                <h2 className="text-orange-500 text-5xl font-semibold">Top Features Listing</h2>
            ...
            <div className="grid md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-4 mx-5">
                {listings.map((listing: Listing, index: number) => (
                    <div key={listing.id} className="bg-white rounded-lg shadow hover:shadow-2xl duration-300 overflow-hidden">
                        <div className="relative">
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img className="w-full h-60 object-cover" src={listing.imageUrl} alt={`${listing.title} Image`} />
